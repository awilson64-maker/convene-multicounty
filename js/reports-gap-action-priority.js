(() => {
  ...
  function init() {
    ...
    window.addEventListener('click', event => {
      ...
      if (generate && document.getElementById('reportType')?.value === 'gap') {
        ...
        buildActionGapReport();
      }
      const print = event.target.closest?.('#reportPrintBtn');
      if (print && state.lastHtml && document.querySelector('#reportsView.view.active')) {
        event.preventDefault();
        event.stopImmediatePropagation();
        window.print();
      }
      const exportBtn = event.target.closest?.('#reportExportBtn');
      if (exportBtn && state.lastHtml && document.querySelector('#reportsView.view.active')) {
        event.preventDefault();
        event.stopImmediatePropagation();
        exportHtml();
      }
  ...
  async function buildActionGapReport() {
    ...
    try {
      ...
      const metricKey = resolveMetricKey(settings);
      ...
      if (scopeMissing) {
        body = `
          ...
          ${section('Choose a service or focus lens', `<div class="report-callout"><p><b>This report needs a selected service type or focus tag.</b> The service/focus lens defines the matching assets. The need layer defines the Census signal. Choose both, then generate the report again.</p></div>`)}
          ${section('Highest-need tracts before asset comparison', needOnlyTable(all.slice(0, 12), metricKey))}`;
      } else {
        const ranked = actionRankedTracts(census, mappedAssets, metricKey);
        const shown = ranked.slice(0, 12);
        const focusFirst = ranked.filter(t => t.actionTier === 1).length;
        const investigate = ranked.filter(t => t.actionTier === 2).length;
        const capacity = ranked.filter(t => t.actionTier === 3).length;
        body = `
          ${metricSection([['Relevant orgs', orgs.length], ['Mapped matching assets', mappedAssets.length], ['Census tracts', all.length], ['Focus first', focusFirst], ['Investigate', investigate], ['Capacity check', capacity]])}
          ${section('How to read this report', `<div class="report-callout"><p><b>The list is now ordered by practical attention.</b> A tract rises to the top when it has both meaningful need and weak matching access. High need with a nearby matching asset is still important, but it is treated as a capacity, eligibility, hours, awareness, or coordination question rather than a geographic access gap.</p></div>`)}
          ${section('Geographic access readout', `<p>This report compares mapped assets matching <b>${escapeHtml(settings.focusTag || settings.serviceType)}</b> against the Census need layer <b>${escapeHtml(metric.label)}</b>. The final focus score uses an interaction model: <b>relative need × access gap</b>. This keeps tracts with very close matching access from dominating the gap list.</p>`)}
          ${section('Recommended focus order', tractTable(shown, metricKey))}
          ${settings.includeCharts ? section('Gap visuals', `<div class="chart-grid">${barChart('Recommended attention order', shown.map(t => [t.shortLabel, t.focusScore]), 12)}${barChart('Access gap score', shown.map(t => [t.shortLabel, t.accessGap]), 12)}</div>`) : ''}
      ...
      state.lastHtml = `<article class="report-doc">
        ...
        ${body}
        <footer class="report-section"><p class="muted small">Generated from the active county workspace. The service/focus selection defines matching assets. The need layer defines the Census signal. Recommended focus order is based on the combination of need and access gap, not need alone.</p></footer>
      </article>`;
      output.innerHTML = state.lastHtml;
    } catch (err) {
      console.error('Action-ranked gap report failed', err);
  ...
  function actionRankedTracts(census, mappedAssets, metricKey) {
    return allTracts(census, metricKey).map(tract => {
      ...
      const focusScore = Math.round((tract.needScore * accessGap) / 100);
      const category = actionCategory(tract.needScore, accessGap, closest?.miles, assetCount);
      return { ...tract, assetCount, closest, accessGap, focusScore, ...category };
    }).sort((a, b) => a.actionTier - b.actionTier || b.focusScore - a.focusScore || b.needScore - a.needScore || b.accessGap - a.accessGap);
  }

  function actionCategory(needScore, accessGap, closestMiles, assetCount) {
    if (needScore >= 35 && accessGap >= 55) return { actionTier: 1, actionLabel: 'Focus first', accessSignal: 'High need + weak matching access' };
    if (needScore >= 18 && accessGap >= 55) return { actionTier: 2, actionLabel: 'Investigate', accessSignal: 'Some need + weak matching access' };
    if (needScore >= 35 && accessGap < 55) return { actionTier: 3, actionLabel: 'Capacity check', accessSignal: 'High need with nearby matching access' };
    if (accessGap >= 55) return { actionTier: 4, actionLabel: 'Monitor geography', accessSignal: 'Weak access, lower need signal' };
    return { actionTier: 5, actionLabel: 'Maybe someday', accessSignal: assetCount ? 'Lower need and some matching access' : 'Lower need, limited mapped access' };
  }
  ...
  function tractTable(tracts, metricKey) {
    const metric = metricOptions[metricKey] || metricOptions.compositePovertyNoVehicle;
    return tracts.length ? table(['Order', 'Tract', 'Category', 'Population', metric.label, 'Relative need', 'Access gap', 'Final focus score', 'Closest matching asset'], tracts.map((t, i) => [
      i + 1,
      t.name,
      t.actionLabel,
  ...
  function needOnlyTable(tracts, metricKey) {
    const metric = metricOptions[metricKey] || metricOptions.compositePovertyNoVehicle;
  ...
  function allTracts(census, metricKey) {
    ...
    const tracts = features.map(feature => {
      ...
      return {
        name: tractLabel(p),
        shortLabel: tractLabel(p).replace('Census Tract ', 'Tract '),
  ...
  function workspace() {
    ...
    return { organizations: arr(raw.organizations).map(normalizeOrg), contacts: arr(raw.contacts), activities: arr(raw.activities).map(normalizeActivity), relationships: arr(raw.relationships) };
  }

  function normalizeOrg(raw = {}) {
    return { ...raw, id: raw.id || raw.orgId || raw.organizationId || '', name: raw.name || raw.organization || '', type: raw.type || raw.category || '', status: raw.status || '', reach: raw.reach || raw.geographicReach || '', confidence: raw.confidence || raw.reachConfidence || '', focus: Array.isArray(raw.focus ?? raw.tags ?? raw.focusTags) ? (raw.focus ?? raw.tags ?? raw.focusTags).join(', ') : String(raw.focus ?? raw.tags ?? raw.focusTags ?? ''), address: raw.address || '', phone: raw.phone || '', email: raw.email || '', website: raw.website || '', lat: cleanNumber(raw.lat ?? raw.latitude), lng: cleanNumber(raw.lng ?? raw.longitude) };
  }

  function normalizeActivity(raw = {}) { return { ...raw, organizationIds: toArray(raw.organizationIds || raw.orgIds || raw.organizations), contactIds: toArray(raw.contactIds || raw.contacts) }; }
  function filterOrgs(orgs, settings) { return orgs.filter(org => (!settings.serviceType || org.type === settings.serviceType) && (!settings.focusTag || tagsOf(org).some(tag => key(tag) === key(settings.focusTag)))).sort((a, b) => String(a.name || '').localeCompare(String(b.name || ''))); }
  function filterActivities(activities, orgs, settings) { const ids = new Set(orgs.map(o => o.id).filter(Boolean)); const win = dateWindow(settings); return activities.filter(activity => { const date = activity.date ? new Date(`${activity.date}T00:00:00`) : null; const dateOk = (!win.start || (date && date >= win.start)) && (!win.end || (date && date <= win.end)); const scopeOk = !ids.size || toArray(activity.organizationIds).some(id => ids.has(id)); return dateOk && scopeOk; }).sort((a, b) => String(b.date || '').localeCompare(String(a.date || ''))); }
  function dateWindow(settings) { if (settings.range === 'all') return {}; if (settings.range === 'custom') return { start: settings.start ? new Date(`${settings.start}T00:00:00`) : null, end: settings.end ? new Date(`${settings.end}T23:59:59`) : null }; const days = Number(settings.range || 0); const end = new Date(); const start = new Date(); start.setDate(start.getDate() - days); start.setHours(0, 0, 0, 0); return { start, end }; }

  function metricSection(items) { return `<section class="report-section"><div class="metric-grid">${items.map(([label, value]) => `<div class="report-metric"><span>${escapeHtml(label)}</span><b>${escapeHtml(String(value ?? 0))}</b></div>`).join('')}</div></section>`; }
  ...
  function relativeNeedScore(value, min, max, inverse) { const n = Number(value); if (!Number.isFinite(n) || !Number.isFinite(min) || !Number.isFinite(max) || min === max) return 0; const scaled = ((n - min) / (max - min)) * 100; return Math.max(0, Math.min(100, Math.round(inverse ? 100 - scaled : scaled))); }
  ...
  function exportHtml() { const county = activeCounty(); const style = document.getElementById('reportsV2Styles')?.textContent || ''; const html = `<!doctype html><html><head><meta charset="utf-8"><title>CONVENE Gap Report</title><style>${style}</style></head><body>${state.lastHtml}</body></html>`; const blob = new Blob([html], { type: 'text/html' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `convene-${county.id}-gap-report-${new Date().toISOString().slice(0, 10)}.html`; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url); }
})();